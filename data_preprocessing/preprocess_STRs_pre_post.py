...
@dask.delayed
def make_and_save_feats(samp_dict, idx, save_dir, n_per_side=500):
	""" Used to make and save features in parallel. """
	seq_mat, seq_string, chr_loc = make_pre_post_features(samp_dict, n_per_side)
		
	# Save feature matrix to disk
	fm_fname = "{}.npy".format(idx)
...
if __name__ == '__main__':
	...
	# Dask stuff for parallelization
	...
	for i in tqdm(range(len(samples))):
		...
		sample_data.append(
			make_and_save_feats(samp_dict, i, save_dir, n_per_side)
		)
	...
	# parallel compute
	sample_data = client.persist(sample_data)
	progress(sample_data)
	sample_data = client.compute(sample_data)
	sample_data = client.gather(sample_data)
